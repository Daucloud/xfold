...
def dot_product_attention_torch(q, k, v, mask=None, bias=None):
    ...
    if mask is not None:
        if mask.dim() == 1:
            m = mask[None, None, None, :].to(dtype=q.dtype)
        elif mask.dim() == 2:
            m = mask[:, None, None, :].to(dtype=q.dtype)
        else:
            raise ValueError("Unsupported mask dim")
        add = (1.0 - m) * -1e9
        attn_mask = add if attn_mask is None else (attn_mask + add)
